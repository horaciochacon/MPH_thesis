...
fwrite(best_k1_summary, paste0(output_dir, "/sensitivity/best_theta/best_k1_summary.csv")) 
fwrite(best_k1, paste0(output_dir, "/sensitivity/best_theta/best_k1.csv")) 
fwrite(best_k1_count, paste0(output_dir, "/sensitivity/best_theta/best_k1_count.csv"))
fwrite(best_k1_theta, paste0(output_dir, "/sensitivity/best_theta/best_k1_theta.csv"))
...
fwrite(best_k2_summary, paste0(output_dir, "/sensitivity/best_theta/best_k2_summary.csv")) 
fwrite(best_k2, paste0(output_dir, "/sensitivity/best_theta/best_k2.csv")) 
fwrite(best_k2_count, paste0(output_dir, "/sensitivity/best_theta/best_k2_count.csv"))
fwrite(best_k2_theta, paste0(output_dir, "/sensitivity/best_theta/best_k2_theta.csv"))
...
fwrite(lowest_k1_we_kij, paste0(output_dir, "/sensitivity/best_theta/lowest_k1_we_kij.csv"))
...
fwrite(lowest_k2_we_kij, paste0(output_dir, "/sensitivity/best_theta/lowest_k2_we_kij.csv"))
...
fwrite(merged_k1, paste0(output_dir, "/sensitivity/best_theta/merged_k1.csv"))
...
fwrite(merged_k2, paste0(output_dir, "/sensitivity/best_theta/merged_k2.csv"))

summary_best_theta <- merged_k1 %>% bind_rows(merged_k2)

fwrite(
  summary_best_theta, 
  paste0(output_dir, "/sensitivity/best_theta/summary_best_theta.csv")
)
...
heatmap_k1 <- best_k1_count %>%
  ...
  scale_fill_viridis(
    option = "inferno",
    direction = -1,
    limits = c(50, max(best_k1_count$n))
    ) +
  add_config_caption(config) +
  theme_minimal()

ggsave(
  filename = paste0(output_dir, "/sensitivity/Plots/Heatmaps/wave1_best.pdf"),
...
heatmap_k2 <- best_k2_count  %>% 
  ...
  scale_fill_viridis(
    option = "inferno",
    direction = -1,
    limits = c(100, max(best_k1_count $n))
    ) +
  add_config_caption(config) +
  theme_minimal()

ggsave(
  filename = paste0(output_dir, "/sensitivity/Plots/Heatmaps/wave2_best.pdf"),
